...
public class Lc1801 {
    public int getNumberOfBacklogOrders(int[][] orders) {
        ...
        for (int[] order : orders) {
            ...
            if (orderType == 0) {
                while (amount > 0 && !sellOrders.isEmpty() && sellOrders.peek()[0] <= price) {
                    ...
                    int sellAmount = Math.min(amount,
                                              sellOrder[1]);
                    amount -= sellAmount;
        ...
        int total = 0;
        for (PriorityQueue<int[]> pq : Arrays.asList(buyOrders,
                                                     sellOrders)) {
            while (!pq.isEmpty()) {
                int[] order = pq.poll();
    ...
    public static void main(String[] args) {
        ...
        Lc1801 lc1801 = new Lc1801();
        int res = lc1801.getNumberOfBacklogOrders(orders);
